...
// ===================== MULTIPLAYER (PeerJS only — no external account needed) =====================
// SharkFeast uses PeerJS's free public broker purely to introduce two browsers to each other
// (like a phone switchboard). No sign-up, no config, no "other site" for you to visit or set up.
// Whoever creates a lobby becomes the host; the host's tab is the source of truth for that room
// and relays state to everyone else who joins with the room CODE. Keep the host's tab open.
...
const AppState = {
    ...
    // Multiplayer (PeerJS — join by room code, no external account needed)
...
// Firebase was removed — SharkFeast now runs purely on PeerJS (no external account/setup
// required). These are kept as no-op stubs so the handful of call sites below don't need
// to be hunted down individually; they simply do nothing now.
function pushLobbyToCloud() {}
function removeLobbyFromCloud() {}
function subscribeToLobby() {}
function unsubscribeLobby() {}
function pushChatToCloud() {}
...
// Room codes work across any two devices with no signup or config.
...
function broadcastTimerTick(turnPlayer) {
    if (!AppState.isRoomHost) return;
    const packet = { type: "timer_tick", timeLeft: AppState.timeLeft, turnPlayer: turnPlayer || null };
...
function handlePeerData(data, fromConn) {
    ...
    if (data.type === "timer_tick") {
        // Lightweight per-second update so guests see the countdown move live,
        // without waiting for (or paying the cost of) a full lobby_state sync.
        if (!AppState.isRoomHost) {
            AppState.timeLeft = data.timeLeft;
            const timerDisplay = document.getElementById("round-timer-display");
            if (timerDisplay && typeof data.timeLeft === "number") {
                const label = data.turnPlayer ? `${data.turnPlayer}'s turn — ${data.timeLeft}s` : `${data.timeLeft}s`;
                timerDisplay.textContent = `⏳ ${label}`;
                timerDisplay.style.color = data.timeLeft <= 3 ? "#ef4444" : "#34d399";
            }
        }
        return;
    }
...
function initEventListeners() {
    document.getElementById("enable-online-btn")?.addEventListener("click", () => {
        alert(
            "How multiplayer works in SharkFeast:\n\n" +
            "1. Create a lobby — you get a room CODE.\n" +
            "2. Share that code with a friend (any device, any browser).\n" +
            "3. They click \"Join Code\" and enter it.\n\n" +
            "You connect to each other directly — no account or external site needed. " +
            "Just keep the host's tab open while others are playing."
        );
    });
    ...
    // Event delegation so Leave Lobby always works after re-entering rooms
...
// Runs the same local cleanup as doLeaveLobby(), but synchronously and safely from
// beforeunload/pagehide — so closing the tab (instead of clicking Leave) still removes
// you from the room, and deletes the room entirely if you were the last one in it.
function cleanupOnUnload() {
    try {
        if (!AppState.currentUser || !AppState.activeLobby) return;
        const username = AppState.currentUser.username;
        const lobbies = JSON.parse(localStorage.getItem("shark_feasts_lobbies") || "[]");
        const cur = lobbies.find(l => l.id === AppState.activeLobby.id || l.code === AppState.activeLobby.code);
        if (cur) {
            cur.players = (cur.players || []).filter(p => p !== username);
            cur.gameParticipants = (cur.gameParticipants || []).filter(p => p !== username);
            if (cur.playerStates) delete cur.playerStates[username];
            if (cur.lobbyMods) cur.lobbyMods = (cur.lobbyMods || []).filter(p => p !== username);
        }
        const remaining = lobbies.filter(l => l.players && l.players.length > 0);
        localStorage.setItem("shark_feasts_lobbies", JSON.stringify(remaining));
        localStorage.removeItem("shark_active_lobby");
    } catch (e) {
        // best-effort only — never block the tab from closing
    }
}

function initRoomCleanupHandlers() {
    window.addEventListener("beforeunload", cleanupOnUnload);
    window.addEventListener("pagehide", cleanupOnUnload);

    // Same-browser, different-tab: keep the lobby browser list fresh if another tab
    // creates/leaves/deletes a room (localStorage "storage" events only fire cross-tab).
    window.addEventListener("storage", (e) => {
        if (e.key === "shark_feasts_lobbies" && !AppState.activeLobby) {
            renderPublicLobbies();
        }
    });
}
...
function handleChatMessage(text) {
    ...
    // Peer network relay (cross-device, no external account needed)
...
function renderLobbySeating() {
    ...
    seatingGrid.innerHTML = `
        ...
        <div style="margin-top:14px; display:flex; justify-content:center; gap:10px; flex-wrap:wrap; align-items:center;">
            ...
            ${!AppState.gameActive ? (
                (lobby.players || []).includes(BOT_NAME)
                  ? (canModerateLobby() ? `<button id="remove-bot-btn" style="background:#6b7280; padding:8px 14px; border:none; border-radius:6px; cursor:pointer; color:white; font-weight:bold;">✕ Remove Bot</button>` : `<span style="color:#94a3b8; font-size:12px;">🤖 ${BOT_NAME} is practicing with you</span>`)
                  : `<button id="add-bot-btn" style="background:#6366f1; padding:8px 14px; border:none; border-radius:6px; cursor:pointer; color:white; font-weight:bold;">+ Add Bot</button>`
            ) : ""}
        </div>
    ...
    document.getElementById("add-bot-btn")?.addEventListener("click", () => { addPracticeBot(); });
    document.getElementById("remove-bot-btn")?.addEventListener("click", () => { removePracticeBot(); });
...
function removePracticeBot() {
    const lobby = AppState.activeLobby;
    if (!lobby) return;
    if (!(lobby.players || []).includes(BOT_NAME)) return;

    // If it's mid-round and currently the bot's turn, stop its pending move first
    if (getCurrentTurnPlayer && getCurrentTurnPlayer() === BOT_NAME) {
        stopRoundTimer();
    }

    lobby.players = (lobby.players || []).filter(p => p !== BOT_NAME);
    lobby.gameParticipants = (lobby.gameParticipants || []).filter(p => p !== BOT_NAME);
    if (lobby.playerStates) delete lobby.playerStates[BOT_NAME];
    if (Array.isArray(lobby.turnOrder)) lobby.turnOrder = lobby.turnOrder.filter(p => p !== BOT_NAME);

    saveAndCleanLobbies();
    renderLobbyUI();
    appendChatMessage("System", `🤖 ${BOT_NAME} was removed from the lobby.`);

    // If that drops us below 2 in an active round, end it gracefully
    if (AppState.gameActive && lobby.gameParticipants.length < 2) {
        AppState.gameActive = false;
        stopRoundTimer();
        appendChatMessage("System", "Round paused — not enough players. Add a friend or the bot back to continue.");
        renderLobbyUI();
    }
}
...
function hostStartRound() {
    ...
    if (lobby.gameParticipants.length < 2) {
        appendChatMessage("System", "Need at least 2 players in the round — invite a friend with your room code, or click \"+ Add Bot\" to practice solo.");
        renderLobbyUI();
        return;
    }
    clearStartCountdown();
...
function handleJoinGameRound() {
    ...
    if (lobby.gameParticipants.length < 2) {
        appendChatMessage("System", "Waiting for at least 1 more player — invite a friend with your room code, or click \"+ Add Bot\" to practice solo.");
        return;
    }
...
function startPlayerTurn() {
    ...
    AppState.roundTimer = setInterval(() => {
        ...
        broadcastTimerTick(getCurrentTurnPlayer());
...
function renderPublicLobbies() {
    ...
    if (publicLobbies.length === 0) {
        list.innerHTML = `<p style="color:#8b949e; text-align:center; padding:10px;">No lobbies created on this device yet. Create one, or ask a friend for their room CODE and use "Join Code" above — that works across any device.</p>`;
        return;
    }
...
window.joinLobbyByCode = async function(code) {
    ...
    // PeerJS — the host must have their lobby tab open for this to work.
    try {
    ...
    if (!lobby) {
        alert("Could not join room " + code + ".\n\nMake sure the host still has the lobby tab open, then try again.");
        return;
    }
    enterLobby(lobby, true);
};
