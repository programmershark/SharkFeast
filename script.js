...
function doLeaveLobby() {
    ...
    // Ensure leave button is never stuck disabled
...
function loadLobbySettingsUI() {
    ...
    if (modeSelect) {
        // Ensure option exists even if HTML is older
    ...
    if (livesRange) {
        livesRange.disabled = false;
        livesRange.value = String(lobby.lives || 3);
    }
...
function saveLobbySettings() {
    ...
    if (livesEl) {
        ...
        // Apply to players not currently mid-round; also update lives val label
        const livesVal = document.getElementById("lobby-lives-val");
    ...
    // Sync into lobbies array
    const idx = AppState.lobbies.findIndex(l => l.id === AppState.activeLobby.id);
    if (idx >= 0) AppState.lobbies[idx] = AppState.activeLobby;

    saveAndCleanLobbies();
    renderLobbyUI();

    // Only one quiet confirmation — no spam if nothing meaningful changed repeatedly
    const changed =
        prev.rules !== AppState.activeLobby.rulesText ||
        prev.mode !== AppState.activeLobby.gameMode ||
        prev.lives !== AppState.activeLobby.lives;
    if (changed) {
        appendChatMessage("System", `⚙️ Settings saved (${AppState.activeLobby.gameMode}, ${AppState.activeLobby.lives} lives).`);
    }
}
...
function handleJoinGameRound() {
    ...
    // Up to 16 players can be in a round (min 2 to start) — NOT limited to 2 total
    ...
    saveAndCleanLobbies();
    renderLobbyUI();
    appendChatMessage("System", `🦈 ${AppState.currentUser.username} joined the round with ${startingLives} lives. (${lobby.gameParticipants.length} in round)`);
    ...
    if (lobby.gameParticipants.length >= 2 && !AppState.gameActive) {
        ...
        appendChatMessage("System", `Waiting for at least 2 players to start (${lobby.gameParticipants.length}/${maxRound} seated)...`);
    }
}
...
function renderPublicLobbies() {
    ...
    // Reload from storage so list stays fresh
    AppState.lobbies = JSON.parse(localStorage.getItem("shark_feasts_lobbies") || "[]");
    saveAndCleanLobbies();
    const publicLobbies = AppState.lobbies.filter(l => !l.isPrivate);

    if (publicLobbies.length === 0) {
        list.innerHTML = `<p style="color:#8b949e; text-align:center; padding:10px;">No public lobbies on this device yet. Create one or join with a code.</p>`;
        return;
    }
    ...
    publicLobbies.forEach(l => {
        ...
        html += `<div style="display:flex; justify-content:space-between; align-items:center; padding:10px; background:#030712; border-radius:6px; margin-bottom:8px; gap:8px; flex-wrap:wrap;">
            <div>
                <strong>${l.name}</strong>
                <span style="color:#8b949e; font-size:12px;"> · Host: ${l.host} · ${l.players?.length || 0} players</span>
                <div style="font-size:11px; color:#38bdf8; margin-top:2px;">Code: ${l.code}</div>
            </div>
