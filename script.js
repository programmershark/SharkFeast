...
function showOnlineStatusBanner() {
    ...
    if (AppState.onlineEnabled) {
        ...
        bar.style.background = "rgba(30,64,175,0.92)";
        bar.style.color = "#dbeafe";
        bar.style.pointerEvents = "auto";
        bar.style.cursor = "pointer";
        bar.innerHTML = '🔵 v1.0 · Create a lobby & share the CODE — friends join from any device (host keeps tab open). Optional: <u>Firebase setup</u>';
        bar.onclick = () => openOnlineSetupModal();
    }
}
...
function renderLobbySeating() {
    ...
    seatingGrid.innerHTML = `
        ...
        <div style="margin-top:14px; display:flex; justify-content:center; gap:10px; flex-wrap:wrap; align-items:center;">
            ${!inGameRound ? `
                <button id="join-game-round-btn" style="background:#22c55e; padding:8px 16px; border:none; border-radius:6px; cursor:pointer; color:white; font-weight:bold;">Join Round</button>
            ` : `
                <span style="color:#34d399; font-weight:bold; padding:8px;">Seated ✓</span>
            ...
            `}
            ${isCurrentUserLobbyHost() && !AppState.gameActive ? `
                ...
                <button id="start-round-btn" style="background:#38bdf8; padding:8px 16px; border:none; border-radius:6px; cursor:pointer; color:#030712; font-weight:bold;">▶ Start Round</button>
            ` : ""}
        </div>
        <div style="text-align:center; margin-top:8px; font-size:11px; color:#64748b;">Mode: <strong style="color:#38bdf8;">${mode}</strong> · Lives: ${lobby.lives || 3} · Cap: ${lobby.maxPlayers || 16}</div>
    `;
    ...
    document.getElementById("start-round-btn")?.addEventListener("click", hostStartRound);
...
function refreshRoundStatusLine() {
    ...
    if (!AppState.gameActive) {
        if (joined === 0) statusLine.textContent = `${mode} — Join round, or host: Add Bot + Start.`;
        else if (joined === 1) statusLine.textContent = `1 seated — add a bot or another player, then Start.`;
        else statusLine.textContent = `${joined} seated — host can press ▶ Start Round (${mode}).`;
...
function addPracticeBot() {
    const lobby = AppState.activeLobby;
    if (!lobby || !isCurrentUserLobbyHost()) return;
    ...
    const me = AppState.currentUser.username;
    if (!lobby.gameParticipants.includes(me)) {
        lobby.gameParticipants.push(me);
        lobby.playerStates[me] = { lives: lobby.lives || 3 };
    }
    saveAndCleanLobbies();
    renderLobbyUI();
    appendChatMessage("System", `🤖 ${BOT_NAME} joined. Press ▶ Start Round when ready.`);
}
...
function hostStartRound() {
    ...
    lobby.gameParticipants.forEach(p => { lobby.playerStates[p] = { lives }; });
    saveAndCleanLobbies();

    const mode = String(lobby.gameMode || "Shark Bomb").trim();
    appendChatMessage("System", `▶ Starting ${mode} with ${lobby.gameParticipants.length} players...`);
...
function handleJoinGameRound() {
    ...
    if (lobby.gameParticipants.length >= maxRound) {
    ...
    saveAndCleanLobbies();
    renderLobbyUI();
    appendChatMessage("System", `🦈 ${AppState.currentUser.username} seated (${startingLives} lives). ${lobby.gameParticipants.length} in round.`);
}
...
function botPlayBombTurn() {
    ...
    const prompt = AppState.currentLetters;
    let candidates = [];
    if (typeof SHARK_WORD_BANK !== "undefined") {
        SHARK_WORD_BANK.forEach(w => {
            if (w.toUpperCase().includes(prompt) && !AppState.usedWords.has(w) && w.length >= 3) candidates.push(w);
        });
    }
    if (!candidates.length) candidates = [("x" + prompt + "y").toLowerCase()];
    const word = candidates[Math.floor(Math.random() * candidates.length)];
    stopRoundTimer();
    if (!word.toUpperCase().includes(prompt) || AppState.usedWords.has(word)) {
        appendChatMessage("System", `❌ ${BOT_NAME} failed!`);
        applyLifeLoss(BOT_NAME);
        return;
    }
    AppState.usedWords.add(word);
    appendChatMessage("System", `✅ ${BOT_NAME} submitted "${word}"!`);
    applyBonusLetters(BOT_NAME, word);
    AppState.currentTurnIndex = (AppState.currentTurnIndex + 1) % Math.max(AppState.turnOrder.length, 1);
    saveAndCleanLobbies();
